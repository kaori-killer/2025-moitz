...
function BottomSheet({
  ...
  const [positionPercent, setPositionPercent] = useState(initialSnapPoint);
  const [isAnimating, setIsAnimating] = useState(false);

  const isDraggingRef = useRef(false);
  const startYRef = useRef(0);
  const startPercentRef = useRef(initialSnapPoint);
  const viewportRef = useRef<number>(getViewportHeight());
  const currentPercentRef = useRef(initialSnapPoint); // 드래그 중 현재 위치 추적
  ...
  const onPointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const target = decideDirectionalSnapTarget(start, current, snapPoints, 2);
